...
"""
Enhanced training pipeline with signal-specific optimizations.
"""
import time
import numpy as np
import torch
...
def train_model(model_type, model_params, datasets, signal_type, window_size, n_neurons,
    ...
    """Train a model with signal-specific optimizations."""
    logger.info(f"Training {model_type} on {signal_type}")
    ...
    # Create model parameters with signal type
    enhanced_params = model_params.copy()
    enhanced_params['signal_type'] = signal_type  # Pass signal type to models

    # Initialize model
    if model_type == 'random_forest':
        from mind.models.classical.random_forest import RandomForestModel
        model = RandomForestModel(**enhanced_params)
    elif model_type == 'svm':
        from mind.models.classical.svm import SVMModel
        model = SVMModel(**enhanced_params)
    elif model_type == 'mlp':
        from mind.models.classical.mlp import MLPModel
        model = MLPModel(**enhanced_params)
    elif model_type == 'fcnn':
        from mind.models.deep.fcnn import FCNNWrapper
        model = FCNNWrapper(
            window_size=window_size,
            n_neurons=n_neurons,
            device=device,
            signal_type=signal_type,  # Pass signal type
            **enhanced_params
        )
    elif model_type == 'cnn':
        from mind.models.deep.cnn import CNNWrapper
    ...
    logger.info(f"Performance: Accuracy={results['metrics']['accuracy']:.4f}, "
