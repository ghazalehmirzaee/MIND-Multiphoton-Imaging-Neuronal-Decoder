    ...
    for signal_type, signal in calcium_signals.items():
        ...
        try:
            ...
            if isinstance(model_or_results, dict) and 'cnn' in model_or_results:
                ...
                # If no importance found, continue to next approach
            ...
            # Try to extract from model
                ...
                try:
                    ...
                    if hasattr(model_or_results, 'get_top_contributing_neurons'):
                        ...
                        importance_dict[signal_type] = (importance, top_indices)
                        logger.info(f"Extracted importance for {signal_type} from CNN model")
                        continue
                    else:
                        # Generic method for other models
                        importance_matrix = model_or_results.get_feature_importance(
                            window_size=calcium_signals[signal_type].shape[0],
                            n_neurons=n_neurons
                        )

                        # Check shape
                        if importance_matrix.ndim != 2:
                            logger.warning(f"Unexpected importance matrix shape: {importance_matrix.shape}")
                            continue
                        ...
                        importance_dict[signal_type] = (importance, top_indices)
                        logger.info(f"Extracted importance for {signal_type} from model feature importance")
                        continue
                except Exception as e:
...
def plot_neuron_bubble_charts(calcium_signals: Dict[str, np.ndarray],
    ...
    """
    Plot bubble charts showing top contributing neurons with aligned indices.
    ...
    fig.suptitle(
        ...
        "Size Indicates Neuron's Contribution to Movement Classification",
...
def plot_top_neuron_bubbles(
    ...
    """
    Create bubble charts for top contributing neurons with robust error handling.
    ...
    -------
    Optional[plt.Figure]
        The figure object if successful, None if failed
    """
    try:
        ...
        # Extract importance with error checking
        try:
            importance_dict = extract_neuron_importance(model_or_results, calcium_signals, top_n)
        except Exception as e:
            logger.error(f"Error extracting neuron importance: {e}")
            raise ValueError(f"Failed to extract neuron importance: {e}")
